    ...
    <div className="mb-16">
      ...
      <div className="relative group">
        ...
        {data.length > 0 ? (
          ...
          >
            {data.map(p => (
              <div key={p.id} className="w-[calc(50%-0.5rem)] sm:w-[calc(33.333%-1rem)] lg:w-[calc(25%-1.125rem)] flex-shrink-0 animate-fade-in-up">
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (pid && products.length > 0) {
      ...
      if (productToOpen && (!selectedProduct || String(selectedProduct.id) !== pid)) {
            ...
            <div className="relative z-10 flex flex-col flex-1 overflow-hidden">
              ...
              <div className="flex items-center justify-between p-4 md:p-5 border-b border-gray-100 bg-slate-50 sticky top-0 z-30 flex-shrink-0 shadow-sm">
                ...
                <div className="flex items-center gap-4 flex-1 overflow-hidden pr-4">
                  <h2 className={`text-lg md:text-xl font-black text-slate-800 uppercase tracking-wide flex items-center gap-2.5 transition-all ${isModalScrolled ? 'hidden sm:flex' : 'flex'}`}>
                  ...
                  <div className={`flex items-center gap-3 transition-all duration-300 origin-left ${isModalScrolled ? 'opacity-100 scale-100 sm:pl-4 sm:border-l border-gray-200' : 'opacity-0 scale-95 hidden'}`}>
                    ...
                    <div className="flex flex-col justify-center min-w-0">
                      ...
                      <p className="text-[11px] font-black text-orange-500 tracking-wider">{formatPrice(selectedProduct.price)}</p>
                    </div>
                  </div>
                </div>
              ...
              >
                        ...
                        <div className="grid grid-cols-2 gap-x-6 gap-y-3.5 text-sm">
                          {selectedProduct.specs.map((spec, i) => (
                            <div key={i} className="flex justify-between items-center border-b border-gray-200 pb-2 last:border-0 last:pb-0">
                              <span className="text-slate-500 font-medium tracking-wide text-xs uppercase">{spec.key || spec.Key}</span>
                              <span className="text-slate-800 font-bold">{spec.value || spec.Value}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                <div className="border-t border-gray-100 pt-16 mb-16 pb-16">
                  <div className="flex items-center justify-between mb-10 gap-4">
                    <h3 className="text-2xl font-black uppercase text-slate-800 tracking-tight flex items-center gap-3"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-7 h-7 text-orange-400"><path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442a.562.562 0 01.313.988l-4.289 3.692a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.385a.563.563 0 00-.182-.557l-4.289-3.692a.562.562 0 01.313-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" /></svg> Đánh giá từ khách hàng ({selectedProduct.reviews ? selectedProduct.reviews.length : 0})</h3>
                  </div>
                ...
                <div className="border-t border-gray-100 pt-10 mt-12 pb-6 relative group">
                  ...
                  <div className="relative group">
                    ...
                    <div ref={relatedCarouselRef} className="flex gap-4 lg:gap-6 overflow-x-auto scroll-smooth pb-6 pt-2 px-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
