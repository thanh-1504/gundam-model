...
const Catalog = () => {
  ...
  // 🔥 STATE MỚI: Bắt trạng thái cuộn của Modal
  ...
  useEffect(() => {
    if (selectedProduct) {
      ...
      setIsModalScrolled(false); // Reset trạng thái cuộn khi mở sp mới
  ...
  useEffect(() => {
    const fetchCatalogData = async () => {
      ...
      try {
        ...
        toast.error("Không thể kết nối đến máy chủ Azure!");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 relative min-h-[600px] items-start p-4 bg-[#0a0a0a]">
      ...
      <aside className="w-full md:w-64 flex-shrink-0 sticky top-24 z-10">
        <div className="bg-[#141414] p-5 rounded-xl border border-gray-800 shadow-xl max-h-[calc(100vh-7rem)] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
          
          <h3 className="text-[15px] font-black uppercase text-white border-b border-gray-700 pb-3 mb-4">Bộ lọc danh mục</h3>
          <div className="space-y-5 mb-8">
            {categories && categories.length > 0 ? (
              categories.map(category => (
                <div key={category.id || category.Id}>
                  <h4 className="font-bold text-gray-400 uppercase tracking-wider text-[11px] mb-2">{category.name || category.Name}</h4>
                  <ul className="space-y-1 border-l border-gray-800 pl-2">
                    {subcategories.filter(sub => (sub.category_id || sub.categoryId || sub.CategoryId) === (category.id || category.Id)).map(sub => (
                      <li key={sub.id || sub.Id}>
                        <button onClick={() => setActiveSubcategory(sub.id || sub.Id)} className={`text-[13px] w-full text-left px-3 py-1.5 rounded-r-lg transition-all duration-200 border-l-2 ${activeSubcategory === (sub.id || sub.Id) ? 'bg-blue-600/10 text-blue-400 font-bold border-blue-500' : 'text-gray-400 border-transparent hover:text-white hover:bg-gray-800'}`}>
                          {sub.name || sub.Name}
                        </button>
                      </li>
                    ))}
              ...
              <div className="text-gray-500 text-xs italic py-2">Đang tải danh mục...</div>
            )}
          </div>

          <h3 className="text-[15px] font-black uppercase text-white border-b border-gray-700 pb-3 mb-4">Lọc theo giá</h3>
          <div className="space-y-2 mb-6 text-sm text-gray-300">
            <label className="flex items-center gap-2 cursor-pointer hover:text-white transition-colors">
              <input type="radio" name="price" value="all" checked={priceRange === 'all'} onChange={(e) => setPriceRange(e.target.value)} className="accent-blue-500 w-4 h-4"/> Tất cả giá
            </label>
            <label className="flex items-center gap-2 cursor-pointer hover:text-white transition-colors">
              <input type="radio" name="price" value="under500" checked={priceRange === 'under500'} onChange={(e) => setPriceRange(e.target.value)} className="accent-blue-500 w-4 h-4"/> Dưới 500.000 đ
            </label>
            <label className="flex items-center gap-2 cursor-pointer hover:text-white transition-colors">
              <input type="radio" name="price" value="500-1m" checked={priceRange === '500-1m'} onChange={(e) => setPriceRange(e.target.value)} className="accent-blue-500 w-4 h-4"/> 500.000đ - 1.000.000đ
            </label>
            <label className="flex items-center gap-2 cursor-pointer hover:text-white transition-colors">
              <input type="radio" name="price" value="over1m" checked={priceRange === 'over1m'} onChange={(e) => setPriceRange(e.target.value)} className="accent-blue-500 w-4 h-4"/> Trên 1.000.000 đ
            </label>
          </div>

          <button onClick={() => { setActiveSubcategory('all'); setPriceRange('all'); setInStockOnly(false); setSearchQuery(''); setCurrentPage(1); }} className="text-[13px] font-bold w-full py-2.5 rounded-lg bg-gray-800 text-white hover:bg-blue-600 transition-all uppercase tracking-wide shadow-lg">Reset Filter</button>
        </div>
      </aside>
      ...
      <main className="flex-grow w-full overflow-hidden">
        ...
        {/* THANH TOOLBAR */}
        <div className="bg-[#141414] p-4 rounded-xl border border-gray-800 mb-6 flex flex-col lg:flex-row justify-between items-center gap-4">
          <div className="relative w-full lg:w-[350px]">
            <input type="text" placeholder="Tìm kiếm mô hình..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-black border border-gray-700 text-sm text-white rounded-lg pl-10 pr-4 py-2 outline-none focus:border-blue-500"/>
          ...
          <div className="flex gap-4 items-center">
            <label className="flex items-center gap-2 cursor-pointer text-sm font-bold text-gray-300 hover:text-green-400 transition-colors bg-gray-900 px-3 py-1.5 rounded-lg border border-gray-700 whitespace-nowrap">
              <input type="checkbox" checked={inStockOnly} onChange={(e) => setInStockOnly(e.target.checked)} className="accent-green-500 w-4 h-4 rounded"/> Sẵn hàng ({filteredProducts?.length || 0})
            </label>
            <select value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} className="bg-black border border-gray-700 text-white text-[13px] rounded-lg px-3 py-2 outline-none cursor-pointer">
        ...
        {totalPages > 1 && (
            <div className="flex justify-center mt-10 gap-2">
                <button onClick={() => { setCurrentPage(prev => Math.max(prev - 1, 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }} disabled={currentPage === 1} className="px-4 py-2 bg-[#141414] text-white text-sm font-bold rounded-lg disabled:opacity-30 hover:bg-gray-800 transition-colors border border-gray-800">&larr; Trước</button>
                <div className="hidden sm:flex gap-2">
                  {[...Array(totalPages)].map((_, i) => (
                      <button key={i} onClick={() => { setCurrentPage(i + 1); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`w-10 h-10 rounded-lg text-sm font-black ${currentPage === i + 1 ? 'bg-blue-600 text-white' : 'bg-[#141414] text-gray-400 hover:bg-gray-800 hover:text-white'}`}>{i + 1}</button>
                  ))}
                </div>
                <button onClick={() => { setCurrentPage(prev => Math.min(prev + 1, totalPages)); window.scrollTo({ top: 0, behavior: 'smooth' }); }} disabled={currentPage === totalPages} className="px-4 py-2 bg-[#141414] text-white text-sm font-bold rounded-lg disabled:opacity-30 hover:bg-gray-800 transition-colors border border-gray-800">Sau &rarr;</button>
            </div>
        )}
      </main>
            ...
            <div className="relative z-10 flex flex-col flex-1 overflow-hidden">
                ...
                <div className="flex items-center justify-between p-4 md:p-5 border-b border-gray-800 bg-[#181818]/95 backdrop-blur-md sticky top-0 z-30 flex-shrink-0 shadow-md transition-all">
                  ...
                  <div className="flex items-center gap-4 flex-1 overflow-hidden pr-4">
                    {/* Chữ CHI TIẾT SẢN PHẨM sẽ bị ẩn đi trên mobile nếu đang cuộn để nhường chỗ cho Tên SP */}
