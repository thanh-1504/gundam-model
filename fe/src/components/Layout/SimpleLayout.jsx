      ...
      <header className={`sticky top-0 z-40 transition-all duration-300 ${isScrolled ? 'bg-black/80 backdrop-blur-lg border-b border-gray-800 shadow-xl' : 'bg-black border-b border-gray-800'}`}>
        <div className="max-w-[1280px] mx-auto px-4 lg:px-8 h-20 flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-6 xl:gap-8 text-[13px] font-bold text-gray-300 uppercase tracking-widest h-full whitespace-nowrap flex-1 justify-center">
            ...
            <a href="https://hobbyjapan-social.vercel.app/social" className="hover:text-white transition-colors h-full flex items-center gap-1.5 group relative border-b-2 border-transparent hover:border-orange-500">
...
{user ? (
  <div className="relative z-50 ml-1" ref={dropdownRef}>
    <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center focus:outline-none group">
      <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-600 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-md border border-gray-600 group-hover:border-blue-400 transition-all">
        {user.username ? user.username.charAt(0).toUpperCase() : 'U'}
      </div>
    </button>

    {/* MENU XỔ XUỐNG KHI CLICK VÀO AVATAR */}
    {isDropdownOpen && (
      <div className="absolute right-0 mt-3 w-56 bg-[#1f1f1f] border border-gray-700 rounded-xl shadow-2xl py-2 overflow-hidden animate-fade-in origin-top-right">
        ...
        <div className="px-4 py-3 border-b border-gray-700 bg-[#141414]">
          <p className="text-sm font-bold text-white line-clamp-1">{user.username || 'Gunpla Pilot'}</p>
          <p className="text-xs text-gray-400 line-clamp-1">{user.email || user.role}</p>
        </div>

        <div className="py-1">
          <Link to="/profile" onClick={() => setIsDropdownOpen(false)} className="block px-4 py-2.5 text-sm font-bold text-gray-300 hover:bg-gray-800 hover:text-white transition-colors flex items-center gap-2.5">
            Thông tin cá nhân
          </Link>
          <Link to="/orders" onClick={() => setIsDropdownOpen(false)} className="block px-4 py-2.5 text-sm font-bold text-gray-300 hover:bg-gray-800 hover:text-white transition-colors flex items-center gap-2.5">
            Lịch sử mua hàng
          </Link>
          {user.role === 'admin' && (
            <Link to="/admin" onClick={() => setIsDropdownOpen(false)} className="block px-4 py-2.5 text-sm font-bold text-orange-400 hover:bg-gray-800 transition-colors border-t border-gray-700 flex items-center gap-2.5 mt-1">
              Trang Quản Trị
            </Link>
          )}
        </div>

        <button onClick={() => { setIsDropdownOpen(false); logout(); }} className="w-full text-left px-4 py-2.5 text-sm font-bold text-red-400 hover:bg-red-500/10 transition-colors flex items-center gap-2.5 border-t border-gray-700">
          Đăng xuất
        </button>
      </div>
    )}
