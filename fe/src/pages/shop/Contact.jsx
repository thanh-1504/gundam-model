...
const Contact = () => {
  ...
  return (
    <div className="bg-slate-50 min-h-screen text-slate-800 w-full py-16 relative overflow-hidden font-sans">
      ...
      <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-blue-100 rounded-full blur-[100px] opacity-60 -translate-x-1/2 -translate-y-1/2 pointer-events-none"></div>
      <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-orange-50 rounded-full blur-[120px] opacity-60 translate-x-1/3 translate-y-1/3 pointer-events-none"></div>
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16">
          ...
          <div className="w-full lg:w-5/12 space-y-8">
            ...
            <div className="space-y-5">
              {/* Địa chỉ */}
              <div className="flex items-start gap-5 p-6 bg-white border border-gray-100 rounded-2xl hover:shadow-md hover:border-blue-100 transition-all duration-300 group">
                <div className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center flex-shrink-0 text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-colors">
          ...
          <div className="w-full lg:w-7/12">
            <div className="bg-white p-8 md:p-10 rounded-3xl border border-gray-100 shadow-xl shadow-slate-200/50">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="text" name="name" value={formData.name} onChange={handleChange} required placeholder="Nhập tên của bạn" className="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3.5 focus:outline-none focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"/>
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Địa chỉ Email</label>
                    <input type="email" name="email" value={formData.email} onChange={handleChange} required placeholder="example@email.com" className="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3.5 focus:outline-none focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"/>
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Chủ đề</label>
                  <input type="text" name="subject" value={formData.subject} onChange={handleChange} required placeholder="Vấn đề bạn cần hỗ trợ" className="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3.5 focus:outline-none focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"/>
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Nội dung tin nhắn</label>
                  <textarea name="message" value={formData.message} onChange={handleChange} required rows="5" placeholder="Chi tiết yêu cầu của bạn..." className="w-full bg-slate-50 border border-gray-200 text-slate-800 rounded-xl px-4 py-3.5 focus:outline-none focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all resize-none"></textarea>
                </div>

                <button type="submit" className="w-full py-4 bg-orange-500 hover:bg-orange-600 text-white font-bold uppercase text-sm tracking-widest rounded-xl transition-all shadow-md shadow-orange-500/20 hover:shadow-lg hover:shadow-orange-500/40 active:scale-[0.98] flex items-center justify-center gap-2 mt-2">
                  Gửi Tin Nhắn
