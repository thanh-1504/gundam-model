    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col relative selection:bg-blue-200 selection:text-blue-900">
        ...
        <div className="max-w-[1280px] mx-auto px-4 lg:px-8 h-16 md:h-20 flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-6 xl:gap-8 text-[14px] font-bold text-white/90 uppercase tracking-wide h-full whitespace-nowrap flex-1 justify-center">
            ...
            <div className="relative group h-full flex items-center cursor-pointer">
              ...
              <div className="absolute top-full left-0 w-64 bg-white border border-gray-100 rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-2 group-hover:translate-y-0 overflow-hidden mt-1">
                <div className="py-2">
            ...
            {user ? (
              <div className="relative z-50 ml-1" ref={dropdownRef}>
                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center focus:outline-none">
                  <div className="w-9 h-9 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-sm shadow-md hover:shadow-lg transition-all hover:-translate-y-0.5">
                    {user.username ? user.username.charAt(0).toUpperCase() : 'U'}
                  </div>
                </button>

                {isDropdownOpen && (
                  <div className="absolute right-0 mt-3 w-56 bg-white border border-gray-100 rounded-2xl shadow-xl py-2 overflow-hidden animate-fade-in origin-top-right">
              ...
              <a href="https://hobbyjapan-social.vercel.app/auth/login" className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2.5 rounded-full uppercase text-xs font-bold transition-all shadow-md hover:shadow-lg hover:shadow-orange-500/30 whitespace-nowrap ml-2">Đăng nhập</a>
            )}
          </div>
        </div>
      </header>
      ...
      <div className={`fixed top-0 left-0 h-full w-[280px] bg-white shadow-2xl z-[60] transform transition-transform duration-300 ease-out flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-2">
          ...
          <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="block p-3 text-sm font-bold text-slate-700 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors">Trang chủ</Link>
          
          <div className="p-3">
             <span className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Danh mục sản phẩm</span>
             <div className="space-y-1">
                <Link to="/shop" onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 px-3 text-sm font-semibold text-slate-600 hover:text-blue-600 hover:bg-slate-50 rounded-lg">Tất cả sản phẩm</Link>
                ...
                {categories.map(cat => (
                  <Link 
                    ...
                    className="block py-2.5 px-3 text-sm font-semibold text-slate-600 hover:text-blue-600 hover:bg-slate-50 rounded-lg"
                  >
                    {cat.name || cat.Name}
                  </Link>
                ))}
          ...
          <Link to="/contact" onClick={() => setIsMobileMenuOpen(false)} className="block p-3 text-sm font-bold text-slate-700 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors">Liên hệ</Link>
        </div>
      </div>
        ...
        <div className="max-w-[1280px] mx-auto px-4 lg:px-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <a href="#" className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-blue-600 hover:text-white transition-colors">
              ...
              <a href="#" className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-900 hover:text-white transition-colors">
          ...
          <div>
            ...
            <div className="flex bg-slate-50 border border-gray-200 rounded-xl overflow-hidden focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
