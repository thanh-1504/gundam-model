    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col relative selection:bg-blue-200 selection:text-blue-900">
        ...
        <div className="max-w-[1280px] mx-auto px-4 lg:px-8 h-16 md:h-20 flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-6 xl:gap-8 text-[14px] font-bold text-white/90 uppercase tracking-wide h-full whitespace-nowrap flex-1 justify-center">
            ...
            <div className="relative group h-full flex items-center cursor-pointer">
              ...
              <div className="absolute top-full left-0 w-64 bg-white border border-gray-100 rounded-md shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-2 group-hover:translate-y-0 overflow-hidden mt-1">
                <div className="py-2">
          ...
          <div className="flex items-center gap-3 xl:gap-5 flex-shrink-0">
            <form onSubmit={handleSearch} className="hidden md:flex relative group w-48 lg:w-64">
              <input type="text" placeholder="Tìm sản phẩm..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/95 border border-white/30 text-sm text-slate-800 rounded-md pl-5 pr-10 py-2.5 focus:outline-none focus:bg-white focus:border-white focus:ring-2 focus:ring-white/40 transition-all shadow-sm"/>
            ...
            <button onClick={() => setIsNotifOpen(true)} className="relative text-white/90 hover:text-white transition-colors">
              ...
              <span className="absolute top-0 right-0.5 bg-white w-2.5 h-2.5 rounded-sm border-2 border-orange-500 animate-pulse"></span>
            </button>
            ...
            >
              ...
              {totalItems > 0 && (
                <span className="absolute -top-1.5 -right-2 bg-white text-orange-600 text-[10px] font-black h-[18px] min-w-[18px] flex items-center justify-center rounded-md px-1 shadow-sm border border-orange-200">
                  {totalItems}
                </span>
              )}
            </button>
            ...
            {user ? (
              <div className="relative z-50 ml-1" ref={dropdownRef}>
                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center focus:outline-none">
                  <div className="w-9 h-9 rounded-md bg-blue-600 flex items-center justify-center text-white font-bold text-sm shadow-md hover:shadow-lg transition-all hover:-translate-y-0.5">
                    {user.username ? user.username.charAt(0).toUpperCase() : 'U'}
                  </div>
                </button>

                {isDropdownOpen && (
                  <div className="absolute right-0 mt-3 w-56 bg-white border border-gray-100 rounded-md shadow-xl py-2 overflow-hidden animate-fade-in origin-top-right">
              ...
              <a href="https://hobbyjapan-social.vercel.app/auth/login" className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2.5 rounded-md uppercase text-xs font-bold transition-all shadow-md hover:shadow-lg hover:shadow-orange-500/30 whitespace-nowrap ml-2">Đăng nhập</a>
            )}
          </div>
        </div>
      </header>
      ...
      {cartNotice && (
        ...
        >
          ...
          ></div>
          <div className="rounded-md border border-orange-100 bg-white shadow-[0_22px_45px_rgba(15,23,42,0.18)] overflow-hidden relative">
            ...
            <div className="px-5 py-4">
              <div className="flex gap-4 items-start mb-4">
                <div className="w-16 h-16 rounded-md border border-gray-200 bg-white overflow-hidden flex-shrink-0 flex items-center justify-center">
              ...
              <div className={`space-y-3 ${cartItems.length > 2 ? 'max-h-[164px] overflow-y-auto pr-2' : ''}`}>
                {cartItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 rounded-md border border-gray-100 bg-slate-50 px-3 py-2.5">
                    <div className="w-12 h-12 rounded-sm border border-gray-200 bg-white overflow-hidden flex-shrink-0 flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
                          <span className="inline-flex items-center rounded-md bg-blue-50 px-2 py-0.5 font-bold text-blue-700 border border-blue-100">
                          SL: {item.quantity}
                        </span>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 flex gap-3">
                <button
                  ...
                  className="flex-1 py-3 rounded-md bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold transition-colors"
                >
                  Xem giỏ hàng
                </button>
                <button
                  onClick={() => { closeCartNotice(); navigate('/checkout'); }}
                  className="flex-1 py-3 rounded-md border border-blue-600 text-blue-600 hover:bg-blue-50 text-sm font-bold transition-colors"
                >
                  Thanh toán
                </button>
              </div>
      ...
      <div className={`fixed top-0 left-0 h-full w-[280px] bg-white shadow-2xl z-[60] transform transition-transform duration-300 ease-out flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-2">
          ...
          <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="block p-3 text-sm font-bold text-slate-700 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors">Trang chủ</Link>
          
          <div className="p-3">
             <span className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Danh mục sản phẩm</span>
             <div className="space-y-1">
                <Link to="/shop" onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 px-3 text-sm font-semibold text-slate-600 hover:text-blue-600 hover:bg-slate-50 rounded-md">Tất cả sản phẩm</Link>
                ...
                {categories.map(cat => (
                  <Link 
                    ...
                    className="block py-2.5 px-3 text-sm font-semibold text-slate-600 hover:text-blue-600 hover:bg-slate-50 rounded-md"
                  >
                    {cat.name || cat.Name}
                  </Link>
                ))}
          ...
          <Link to="/contact" onClick={() => setIsMobileMenuOpen(false)} className="block p-3 text-sm font-bold text-slate-700 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors">Liên hệ</Link>
        </div>
      </div>
        ...
        <div className="max-w-[1280px] mx-auto px-4 lg:px-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <a href="#" className="w-9 h-9 rounded-md bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-blue-600 hover:text-white transition-colors">
              ...
              <a href="#" className="w-9 h-9 rounded-md bg-slate-100 flex items-center justify-center text-slate-500 hover:bg-slate-900 hover:text-white transition-colors">
          ...
          <div>
            ...
            <div className="flex bg-slate-50 border border-gray-200 rounded-md overflow-hidden focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
